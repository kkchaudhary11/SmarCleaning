...
///////////////Constants////////////////
const float voltageBatCharged = 11.36; // Voltage measured when battery fully charged //Change this
...
void batteryControl(int input){
  ...
  if(v_battery<=11){ //battery limit of discharge, Don't put this limit lower than  11.1V or you can kill the battery
    control = false;
    }
  else {
    //Do nothing Convention
...
void loop(){
  ...
  bumperState1 = digitalRead(bumper2);
  ...
  if (control){
    ...
    if (sdSHARP(SD1)<=4.3 ){ 
      ...
      else if (sdSHARP(SD3)<=4.3){
        //If the wall is too close avoid it 
        rightMotors(100);
        
      }
    else if (sdSHARP(SD4)<=4.3){
        //If the wall is too close avoid it 
        leftMotors(100);
    ...
    while(digitalRead(wifi)==LOW){
        Serial.print("STOP signal form WiFi");
